...
| `WN_ADRESSAGE_COURRIER` | `true` | **lettre d’adressage sur signal d’alerte** ([[D-218]]) — `GET/POST /api/praticien/adressage/courrier` et le geste au cockpit, dans le bloc « ce qui suspend la décision » | fermé — neuf et éteint à la livraison, **POSÉ en Production le 2026-09-17** (`env-set` + redémarrage, geste du responsable ; constaté par sonde : la route POST rend `400 Corps de requête illisible` là où le drapeau fermé rendait `503`, la garde de drapeau étant la toute première instruction). Le fil médecin n’a aucun drapeau : celui-ci en a un parce que la route **produit** un document qui nomme des signaux d’alerte déclarés et part vers un tiers. Si la revue RGPD du 2026-10-21 imposait une suspension, il n’existerait sinon aucun geste d’exploitation pour la produire — il faudrait un déploiement. Le GET ne nomme aucun dossier : il ne dit que l’état du drapeau, et n’écrit donc aucune ligne au journal d’accès. **Ne suffit pas à lui seul** : la cotation des signaux doit être signée (`tableSignauxSecuriteSignee`), sans quoi la route refuse en 409 — verrou inverse des autres tables, et c’est voulu ([[D-099]]) |
| `WN_TRACE_FORMULATION_CHOIX` | `true` | **l'écriture de `formulation_version`** sur `trust_choice_events` ([[D-222]] §3 fermé) — la version du texte de « Mes choix » RÉELLEMENT servi au patient quand il choisit | **POSÉ en Production le 2026-09-17 à 19:24 UTC**, après la migration `20260917100000_trace_formulation_choix` appliquée par `release-db` (run 35258773789, approuvé, réussi à 19:09:03 UTC) et **CONSTATÉE par conteneur** (`one-off-8295` : « Database schema is up to date! », 86 migrations, aucune en attente) — jamais déduite du run. Effectivité constatée comme l'exige le § B.4 : `env` relu (`true`) et **les deux conteneurs web recréés à 19:25:44 UTC** ; un `env-set` seul ne change rien tant que les conteneurs tournent. **LA QUATRIÈME PREUVE MANQUE, ET IL FAUT LE DIRE** : les routes `trust/choix` et `trust/etat` relisent l'identité AVANT le drapeau, donc un appel anonyme rend 401 dans les deux états — seule une session patient réelle verrait la différence. Même piège que `WN_PORTAIL_JOURNAL` au § B.4. | fermé avant cette date — **NEUF ET ÉTEINT à la livraison**, et il ne garde pas une fonctionnalité : il garde une **colonne**. Le code se déploie AVANT que `release-db` ait appliqué la migration `20260917100000_trace_formulation_choix` ([[D-087]]) ; nommer la colonne avant son existence casserait l'enregistrement du choix du patient — le geste le plus sensible du portail. C'est la règle écrite du dépôt : « un ADD se protège par drapeau éteint » (précédent `WN_EI_INTERRUPTION`). **ORDRE DE POSE, et il ne se raccourcit pas** : migration appliquée, **CONSTATÉE par conteneur**, puis seulement le drapeau. Éteint, la route écrit exactement ce qu'elle écrivait avant — aucune régression, seulement une trace en moins |
| `WN_ASSIETTES_INDIQUEES` | `true` | **les assiettes indiquées servies au cockpit praticien** ([[D-237]]) — `GET /api/praticien/assiettes-indiquees` et la carte du même nom, dans la sous-vue Protocole de la phase Actions | **POSÉ en Production le 2026-09-20 à 08:11 UTC** — geste d'exploitation du cabinet, dans l'ordre que `D-237` §9 impose et qui ne se raccourcit pas : code en ligne vérifié AVANT par **contenance** (`f4512705` déployé deux fois, `success` — 02:02:42 par la release, 02:08:24 par l'auto-deploy), `env-set` posé **après avoir relu que la variable était ABSENTE**, puis **les deux conteneurs web recréés à 08:11:48 UTC** et lus six secondes après par `scalingo ps` — un `env-set` seul ne change rien tant que les conteneurs tournent. Sonde d'environnement dans un one-off démarré APRÈS la pose : `true`. ✅ **CONSTATÉ LE 2026-09-21 à 05:31 UTC — par le COMPORTEMENT, jamais par `env`.** Le journal d'accès porte `/api/praticien/assiettes-indiquees` : **5 lectures servies sur 2 dossiers**, première à 05:31:27.925, dernière à 05:38:12.129 UTC (07:38 au cabinet), relu par conteneur détaché à 07:39 UTC — le journal compte alors **44 routes distinctes** et 4 483 lignes. C'est exactement la sonde posée : verrou fermé, la route répond **avant** `verifierAppartenancePatient` et n'écrit rien ; verrou ouvert, chaque lecture servie écrit sa ligne. La ligne existe, donc le drapeau sert. **CE QUE LA CARTE A SERVI**, mesure d'ÉCRAN rapportée par la session d'origine et non relue en base : six assiettes sur un dossier — trois par `Q_INF_03 (DA) : score 11 ≥ 10`, trois par `Q_GAS_01 : zone warning` —, aucune ligne retirée faute de claim, ce qui concorde avec 20 claims valides sur 20 demandés. Et `Q_GAS_01` s'allume **bien qu'absent de tout pack actif** : ce dossier est l'un des cinq qui l'ont passé. ⚠️ **CE QUI A PRÉCÉDÉ CE CONSTAT, ET QUI RESTE LA LEÇON.** Du 2026-09-20 08:11 UTC au 2026-09-21 05:31 UTC, soit **21 heures**, le verdict a été « POSÉ, NON CONSTATÉ » — et il fallait écrire *pourquoi*. La quatrième preuve manque, et la raison n'est pas une panne : la route teste la **session avant le verrou**, donc un appel anonyme rend `401` dans les deux états — aucune sonde non authentifiée ne distingue quoi que ce soit, et il est inutile d'en chercher une. Le seul témoin est la ligne que `verifierAppartenancePatient` écrit au journal d'accès quand un praticien ouvre la sous-vue Protocole. Relu le **2026-09-20 à 11:18 UTC** par conteneur détaché — `select count(*), max(cree_le) from journal_acces_dossiers`, plus le même compte filtré sur la route ; **l'identifiant du one-off n'est pas cité comme preuve, Scalingo les RÉATTRIBUE** (`one-off-746` a rendu deux conteneurs distincts, du 2026-09-11 et du 2026-09-20, sous un même filtre de logs). L'ancre est l'horodatage et la requête. Résultat : `journal_acces_dossiers` porte **4 450 lignes**, **zéro** pour `/api/praticien/assiettes-indiquees` — mais le **dernier geste praticien du journal ENTIER date du 2026-09-19 à 11:21**, soit AVANT la pose. **Cette absence ne dit donc rien du drapeau : elle dit que personne n'a ouvert de dossier depuis.** Une sonde sans témoin ne prouve rien, et c'est la leçon que `WN_SYNTHESE_PAR_RIDEAU` a coûté 1 h 50 d'aveuglement — voir l'amendement du 2026-09-19 à [[D-226]]. **Le constat attendait une SESSION praticien, pas un correctif** — et c'est elle qui est venue, 21 heures après la pose. La formule se garde pour le prochain drapeau sans surface anonyme : tant que la ligne n'existe pas, ne pas écrire « constaté » ; et ne pas écrire « ça ne marche pas » non plus. **CE QUE LA CARTE POURRA SERVIR, mesuré le 2026-09-21** (agrégats, conteneur détaché) : la base porte **8 packs et un SEUL actif** — celui par défaut, 5 qids, **sans `Q_GAS_01`** ; `PACK_SOCLE_INIT` le porte mais est `actif=f`, comme les sept thématiques. `Q_GAS_01` n'est donc dans **aucun pack actif**, ce qui tranche la contradiction apparente entre la doctrine de [[D-236]] et `QUESTIONNAIRE_OVERRIDES` : les deux disaient vrai sur **deux objets différents**. La borne d'âge de la ligne protéinée touche **1 dossier sur 29** (15 n'ont aucune date de naissance). Rapporté par la session d'origine et **non relu ici** : `Q_INF_03` 18 patients, `Q_GAS_01` 5, `Q_GEO_02` **0 passation**, et 20 claims demandés / 20 valides — donc `retireesFauteDeClaim = 0`. | fermé avant cette date — **NEUF ET ÉTEINT à la livraison, et il est LA SEULE CHOSE entre la table et l'écran.** D'ordinaire un drapeau neuf double un verrou clinique encore fermé ; ici la signature est OUVERTE depuis le 2026-09-19 ([[D-236]]), si bien que sans ce drapeau les sept lignes publiées atteindraient l'écran **au prochain déploiement de production** — une mise en service par accident de calendrier, sur des dossiers réels. Le drapeau rend le jour au cabinet. **Ne suffit pas à lui seul** : `assiettesIndiqueesActives()` est un ET avec `indicationsAssiettesSignees()`, et re-signer la table (le chantier de la porte biologique le fera) rouvre la question. **CE QU'IL N'OUVRE PAS, ET C'EST L'INVARIANT** : **rien ne s'écrit AU DOSSIER CLINIQUE**, rien ne s'assigne, rien ne part vers un patient — la route n'expose **aucun POST**. ⚠️ **« La carte ne porte aucun bouton » a cessé d'être vrai le 2026-09-21** ([[D-240]]) : elle porte désormais « Retenir pour le protocole » sur chaque ligne indiquée. **L'invariant, lui, n'a pas bougé** — ce bouton ne fait que remonter un choix au cockpit ; il faut ensuite l'insérer au constructeur, puis enregistrer la version, et c'est cette troisième route (`POST /api/praticien/protocoles/versions`, garde de registre anxiogène comprise) qui écrit. La carte, elle, n'écrit toujours rien. **UNE ÉCRITURE EXISTE POURTANT, ET LA NOMMER ICI EST LE POINT** : le drapeau posé, chaque GET servi passe par `verifierAppartenancePatient`, qui **écrit une ligne au journal d'accès** (`journalAcces.ts` : un `create`, plus un `deleteMany` de purge opportuniste au-delà de la rétention). C'est le journal G-TRUST-04, voulu, et identique à celui de toute lecture praticien — mais une ligne d'exploitation qui annonce « rien ne s'écrit » à celui qui s'apprête à poser le drapeau est fausse. Constat de revue, corrigé avant la pose. Ce que le praticien fait d'une assiette indiquée reste sa décision, par les gestes qui existaient déjà. Verrou fermé, la route répond **avant** `verifierAppartenancePatient` : aucun accès au dossier n'est journalisé pour une lecture qui n'a pas eu lieu. **CE QUI PEUT RÉDUIRE LA CARTE APRÈS LA POSE, SANS TOUCHER AU DRAPEAU** : un claim qui cesse d'être valide au corpus — une décision de curation suffit — retire SA ligne du service, la table restant signée. La carte le **dit** désormais (`retireesFauteDeClaim`) ; avant ce terme, la fermeture se lisait comme un dossier qui ne déclenche rien |
...
| ~~`WN_SYNTHESE_PAR_RIDEAU`~~ | **RETIRÉ** | ~~la génération automatique d'un brouillon de synthèse à la fermeture d'un rideau~~ | **RETIRÉ LE 2026-09-17 ([[D-226]]), VARIABLE PUIS CODE.** `env-unset` en production, les deux conteneurs web **recréés à 18:49:15 UTC** (20:49:15 heure locale ; ils tournaient depuis 17:25:44 UTC), puis retrait de `declencheurRideau.ts`, de son appel dans `api/patient/submit`, de `isSyntheseParRideauEnabled` et de cette garde. **Le motif n'est pas le coût des appels au modèle** : le brouillon automatique naissait à la soumission du patient, donc AVANT la lecture du praticien, et éteignait ainsi la carte « Synthèse à générer » du Fil — qui se tait tant qu'une synthèse est plus récente que la dernière lecture. L'automatisme supprimait l'invitation qu'il prétendait devancer. Mesure corrigée le 2026-09-19 (one-off, agrégats, [[D-125]]) : **9 brouillons automatiques sur 5 dossiers, 4 rejetés**, contre 6 rejets sur 60 générations manuelles — soit 44 % contre 10 %, et non les « 8 / 50 % contre 3,6 % » lus la veille. **AUCUNE SONDE NE CONSTATE CETTE EXTINCTION** — le drapeau gardait un travail de fond sans surface publique, contrairement à `WN_ADRESSAGE_COURRIER` : le constat est la variable absente de `env` plus la recréation des conteneurs, **et ce constat s'est révélé FAUX** : une ligne `auto_rideau_second` a été produite le 2026-09-17 à **20:39:52 UTC**, 1 h 50 APRÈS la recréation des conteneurs. Ce qui a fermé la porte est le retrait du code, déployé à 22:08:04 UTC ; la cause de la non-propagation n'est pas établie. **Variable absente et conteneurs recréés ne constatent pas l'effet d'un drapeau** — voir l'amendement du 2026-09-19 à [[D-226]]. Ligne conservée parce que ce fichier garde l'histoire de ses drapeaux ; **ne pas réintroduire le nom sans relire [[D-226]] § 1**. Ce qui suit décrit l'état ANTÉRIEUR au retrait : [[D-174]]. **NEUVIÈME drapeau neuf et éteint, et le seul de la série qui déclenche un APPEL AU MODÈLE que personne n'a demandé** : ce n'est pas une surface qui s'ouvre, c'est une machine qui se met à produire sur des dossiers réels sans geste humain en amont. **CE QU'IL N'OUVRE PAS, ET C'EST L'INVARIANT** : rien n'atteint le patient — ce qui est produit est un `Brouillon_IA`, la validation et l'envoi restent deux gestes du praticien. Idempotent par marqueur (`donneesEntree.source`) : deux générations par dossier au maximum, et un brouillon rejeté ne se régénère pas. **POSÉ en Production le 2026-09-12 à 18:08:24 UTC**, et **APRÈS** la validation du responsable — l'ordre que l'épisode `WN_CB_RESULTS_ENABLED` du 2026-09-09 avait fait regretter. Séquence : § 2 bis du dossier RGPD rédigé, soumis, **validé le 2026-09-12** (« l'envoi du booklet est toujours la main du praticien ; la machine n'envoie jamais rien automatiquement »), **puis** pose. Effectivité constatée comme l'exige le § B.4 : code en ligne vérifié AVANT par contenance (`merge-base --is-ancestor`), `env` relu (`true`), **les deux conteneurs web recréés à 18:08:24**. |
...
| `WN_ENABLE_ORIENTATION_NNPP2` | `1` | `tableSignee()` (5 termes depuis `D-067` : validation + date non nulle + forme ISO canonique + claims + concordance `shaPerimetre`) | **20 règles**, `validationExterne: true` depuis le 2026-08-04, et **drapeau POSÉ en Production** — constaté le 2026-08-18 par le comportement ([[D-074]]) → **les deux conditions sont remplies ; l'orientation SERT**. **RELU SUR SCALINGO le 2026-09-13** : `scalingo --app wellneuro env` rend `WN_ENABLE_ORIENTATION_NNPP2=1`. La preuve d'origine était antérieure à la bascule d'hébergeur du 2026-08-22 et constatée **sur Vercel** ; elle ne valait donc plus pour la plateforme courante, et cette ligne ne portait aucune mention de relecture là où dix autres en portent une. |
| `WN_ENABLE_CONTRADICTIONS_NNPP2` | `1` | `tableSignee()` de `contradictionsService.ts` (5 termes depuis `D-067` : validation + date non nulle + forme ISO canonique + claims + concordance `shaPerimetre`) | **1 règle publiée (C-STR)**, table **signée le 2026-08-15** ([[D-061]]) et **drapeau posé en Production le 2026-08-16** ([[D-064]]) → **les deux conditions sont remplies ; les constats sortent au prochain déploiement de production**. L'affichage est câblé depuis [[D-050]] (route cockpit → panneau). **RELU SUR SCALINGO le 2026-09-13** : `scalingo --app wellneuro env` rend `WN_ENABLE_CONTRADICTIONS_NNPP2=1`. CE QUE CETTE RELECTURE TRANCHE AU-DELÀ DE CE DRAPEAU : `tableArretExploitable()` vaut `tableArretSignee() && contradictionsActives()`, donc l'**exclusion `dejaRepondu` de l'orientation MORD réellement en production** — une cible couverte par une passation exploitable et `VALID` n'est pas produite du tout. Ce n'était jusqu'ici qu'une lecture de code. |
