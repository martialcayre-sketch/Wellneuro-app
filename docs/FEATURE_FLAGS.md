...
|---|---|---|---|
| `WN_ENABLE_CORPUS_CLINIQUE_V1` | `1` | `CORPUS_CLINIQUE_SIGNE` (4 termes depuis [[D-083]] : validation + date non nulle + forme ISO canonique + concordance `shaPerimetre` — pas de terme claims, le corpus n'en cite aucun) | **signée le 2026-08-22** ([[D-082]] — validation clinique du responsable, contenu inchangé ; ancrage `shaPerimetre` posé le même jour, [[D-083]]). **Drapeau NON posé à la signature** : il se pose après le déploiement qui la porte, et cette ligne ne dira « posé » qu'une fois la pose **constatée par le comportement** (discipline [[D-074]]). D'ici là : fermé |
