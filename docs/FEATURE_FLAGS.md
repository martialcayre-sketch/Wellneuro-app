...
|---|---|---|---|
| `WN_ENABLE_CORPUS_CLINIQUE_V1` | `1` | `CORPUS_CLINIQUE_SIGNE` (4 termes depuis [[D-083]] : validation + date non nulle + forme ISO canonique + concordance `shaPerimetre` — pas de terme claims, le corpus n'en cite aucun) | **signée le 2026-08-22** ([[D-082]] — validation clinique du responsable, contenu inchangé ; ancrage `shaPerimetre` posé le même jour, [[D-083]]). **Pose déclarée le 2026-08-22 sur Scalingo**, dans l'ordre exigé : build portant la signature déployé à 09:21 UTC, `env-set` ensuite, conteneurs recréés à 09:36 UTC (constat `ps`) — le corpus est **armé** : il sert à la prochaine synthèse. **Constatation par le comportement en attente** ([[D-074]]) : cette ligne ne dira « posé, constaté » qu'après une synthèse réelle portant le référentiel |
