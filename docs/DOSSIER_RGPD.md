...
| **Mesure d'usage — aucune donnée personnelle** | `CompteurOuvertureSources` (un JOUR, une espèce parmi deux, un compte entier) | **Déclarée ici pour que l'absence soit DITE, pas seulement tacite.** Le banc `rubrique5.modeles.test.ts` ne vérifie que les tables filles de `Patient` : celle-ci n'en est pas une, donc il se tait — et une ligne absente ne ment pas, elle se tait. Elle compte combien de fois le panneau « Voir les sources et limites » est affiché puis ouvert, et **rien d'autre** : ni `id_patient`, ni identité de praticien, ni instant, ni ligne par événement. Trois colonnes, un agrégat par jour. « Ce praticien n'ouvre jamais les limitations » et « sur quels dossiers le panneau reste fermé » sont deux phrases structurellement sans réponse, et un contrat négatif garde cette forme (`compteur_ouverture_sources_v1_negatif.sql`, liste blanche de colonnes). La position retenue est qu'elle ne porte aucune donnée personnelle ; **cette qualification appartient au responsable de traitement et n'est pas posée ici** (`D-191`) |
