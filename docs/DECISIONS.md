...
- Conséquences : **la voie lente est inchangée** — chaque claim reste soumis à la validation praticien individuelle avant d'exister pour le moteur (barrière **D-003**) ; l'amendement élargit ce qui est *proposé* à la validation, jamais ce qui la contourne. **La quarantaine sanitaire n'est pas un domaine et n'est pas levée** : une source `lifecycleStatus: quarantined` reste exclue quelle que soit sa thématique — WN-SRC-0318 (« premières ordonnances », vigilance Élevée), WN-SRC-0322 (sevrage benzodiazépines) et WN-SRC-0389 (Alzheimer) restent dehors tant que leur relecture scientifique n'est pas faite. Cette distinction est matérialisée dans `tools/corpus/claims/lib/filtre-orientation.mjs` et éprouvée par deux bancs. Matérialisation en base : migration `20260801200000_rag_claim_usage_orientation` (marquage `metadata.usage = 'orientation'` sur 1 716 claims de 85 sources, quarantaine et perfusion épargnées — vérifié en production le 2026-08-02).
- Réserves : le périmètre est **figé dans une liste** au 2026-08-01 ; sa dérive est surveillée par `tools/corpus/claims/lib/perimetre-orientation.test.mjs`, qui échoue dès que le registre s'en écarte — mais **aucun chemin de démarquage n'existe** : une source entrant en quarantaine après coup garde sa marque en base jusqu'à décision explicite.
