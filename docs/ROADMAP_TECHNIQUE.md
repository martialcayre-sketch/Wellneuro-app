...
> Définition de **R6** tranchée le 2026-07-10 (R10, point 1) : c'est celle de `.claude/skills/wn-r6/SKILL.md` qui fait foi (stabilisation build/tests/go-no-go). Le contenu « moteur clinique avancé » (priorisation, protocoles 21 jours, boussole alimentaire, compléments clean label) précédemment décrit sous R6 n'est pas perdu : il est déjà couvert par les modules produit de [docs/ROADMAP_PRODUIT.md](docs/ROADMAP_PRODUIT.md) (série D/R — protocole 21j, arborescence `corpus/`).
