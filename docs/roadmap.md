...
> Incohérence documentaire relevée en marge de cette revue, non corrigée ici : la définition de **R6** diverge entre ce fichier (« préparation du moteur clinique avancé ») et `.claude/skills/wn-r6/SKILL.md` (« stabilisation build, tests, go/no-go »). À trancher par l'utilisateur avant que R6 ne soit lancé.
...
| **R10** | Décisions produit en suspens avant R5/R6 (arbitrages à prendre, pas des bugs) : (1) trancher la définition de R6 (ce fichier vs `wn-r6/SKILL.md`) avant de le lancer ; (2) exposer ou non le lien portail patient dans le frontend praticien (aujourd'hui envoi par email uniquement) ; (3) calendrier de décommission de `packs.qids` (le registre relationnel est en lecture primaire depuis R3, `qids` reste la source d'écriture praticien — proposer une date de revisite plutôt que trancher immédiatement) ; (4) `QuestionnaireDefinition.niveau`/`.publicCible` : à curer seulement si un usage applicatif apparaît — statut « surveillance », pas action requise maintenant | ⏳ À faire |
