...
| Bibliothèque de biologie fonctionnelle (987 actes NABM V105) | **aucune — dormante** | Régime de remboursement d’un acte de biologie proposé. | WN_CB_ENABLED, WN_CB_PROPOSITION, WN_CB_RESULTS_ENABLED | — | non | a_brancher (2026-08-05) |
...
| Corpus clinique de synthèse V1 | `web/src/app/api/praticien/synthese/route.ts` (route-api)<br>+ 12 indirecte(s) | Cadrage clinique injecté dans la synthèse rédigée par le modèle. | WN_ENABLE_CORPUS_CLINIQUE_V1, WN_SYNTHESE_STREAM | validationExterne | oui | — |
| Table d’indications biologiques (15 règles signées) | 1 surface(s) indirecte(s) seulement | Proposition de bilan hiérarchisée et sourcée, servie au cockpit praticien. | WN_CB_ENABLED, WN_CB_PROPOSITION, WN_CB_RESULTS_ENABLED | validationExterne | non | — |
...
21 source(s) recensée(s), dont **6 dormante(s)**.
