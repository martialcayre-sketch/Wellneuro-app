...
| 3 | Doctrine exécutable | `2026-08-18-doctrine-executable/` | **CADRÉE le 2026-08-23** — huit lots, `CAMPAGNE.md` écrit sur un état réel frais ; l'ouverture reste un geste du responsable. Le cadrage a **déplacé trois véhicules** : V1 est à moitié livré (l'objet à trois formes existe depuis `D-041`/`D-044` ; restent producteurs et politique), **V4 est périmé** (ses deux fiches d'accueil sont livrées avec la chaîne T0 — `DC-39`/`DC-41` deviennent des dettes sans véhicule), et **§D est clos** par le Socle LOT-02 (`D-083`). V2 reste la migration (release-db entre V2 et V3). **V3 (gates de population) n'est plus préventif mais en retard** : `priorityRulesV1` est signée et `D-093` ouvre les recommandations élargies en notant que le classement n'est couvert par aucune ligne signée — sa fenêtre est la borne de six semaines de `D-093`. Bloque toujours toute calibration (barème alimentaire, classements), quelle que soit la date d'arrivée des données 21 j. |
