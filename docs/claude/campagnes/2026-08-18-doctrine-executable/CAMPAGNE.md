...
| LOT-07 | `DC-22` — le total de « Mon équilibre » a-t-il un sens ? | **terminé** (2026-08-24, `D-106` — la question posée avec sa mesure, réponse **NON** : aucune règle ne lit le total, `GLOBAL_BALANCE` n'est qu'un code de provenance que **rien ne cite**, et le patient n'en voit pas le chiffre — mais sa VARIATION était présentée aux deux surfaces, et un libellé patient l'annonçait « en progression ». Issue retenue : la seconde branche de `DC-22` — le total n'est pas retiré, il est **identifié** (`DC-20`). Adjacents : `0,34` et `50` **validés tels quels**, sans bump de version ; l'égalité entre besoins d'une strate **motivée** (`DC-21`)) | — |
...
| LOT-12 | Contre-revue adverse — trois bancs élargis, un texte patient corrigé | **terminé** (2026-08-24, `D-108` — passe Codex sur `16d93c57~1..7793a4ac`, **sept** des treize affirmations réfutées, six debout ; les six trouvailles revérifiées une par une dans l'arbre, toutes réelles. Le trou qui n'était pas un banc perfectible : `components/patient-companion` servait du vocabulaire de jeu **au patient depuis le 2026-07-18**, jamais déclaré aux surfaces surveillées — le garde connaissait la page du portail, pas le composant qu'elle monte. Correctif par CLASSE : un cas remonte **transitivement** les imports du portail et exige que chaque racine atteinte soit déclarée. Trois bancs élargis plutôt que déclarés limités — bump (sorties **et** empreinte du mapping, par version, plus un scénario mesuré sur la frontière d'arrondi), seuils (position d'**écrêtage** ; `.slice` déclaré dehors, avec sa mesure de 30 occurrences), nature du total (alias résolus **à point fixe** + second détecteur **par libellé**). Deux surfaces praticien reçoivent la mention. La borne de `scinderSousPlafond` est **épinglée, pas corrigée** : couper fabriquerait des mots absents d'un texte signé. Résistent : `A1` verrous fail-closed, `A2` `DC-12`, `A3` prédicat unique, `A4` verrous à sens inverse, `A6` fixtures, `D2` valence. Aucun seuil, aucune migration) | LOT-03, LOT-07, LOT-11 |
