...
- Correction apportée pendant la revue (PR #35) : l'audit initial LOT-00 affirmait à tort que `SynthesePanel.tsx` restait sur les variables legacy `--primary`/`--accent`. Vérifié par `grep` : aucun consommateur de ces variables ne subsiste nulle part dans `web/src` — `SynthesePanel.tsx` n'utilise que des tokens sémantiques et hérite donc automatiquement du praticien clair, sans action requise. `AUDIT_UI_REEL.md` et `MATRICE_ECRANS_MIGRATION.md` corrigés en conséquence.
- Correction apportée pendant la revue : le badge d'icône inactif du rail (`SidebarRail.tsx`, `MobileBottomNav.tsx`) utilisait par erreur le même token que son conteneur (`bg-rail-surface` des deux côtés), le rendant invisible. Ajout de `--rail-muted`/`bg-rail-muted` (distinct de `--rail-surface`, reprenant l'ancien `--muted` praticien) pour l'icône inactive, et correction des états `hover` (`hover:bg-rail` au lieu de `hover:bg-rail-surface`) pour rester visibles sur leur conteneur.
