...
/**
 * La composition des deux rideaux vit dans un module FEUILLE depuis le
 * 2026-09-12, et ce fichier la ré-exporte — la table clinique de [[D-052]]
 * reste unique, et ses lecteurs existants n'ont pas bougé d'un caractère.
 *
 * Le motif est le même que pour `clinical/stopRulesLibelles.ts` : un composant
 * client a besoin de la liste, et ce module-ci tire Prisma par
 * `orientationService`. La doctrine qui accompagne les deux constantes a suivi
 * la constante ; elle se lit dans `rideauT0.ts`.
 */
export { RIDEAU_T0, HORS_RIDEAU_MOTIVE, estDuRideauT0 };
