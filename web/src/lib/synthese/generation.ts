// LA GÉNÉRATION D'UNE SYNTHÈSE, HORS DE SA ROUTE.
//
// POURQUOI CE MODULE EXISTE. Tout ce qui suit vivait dans
// `app/api/praticien/synthese/route.ts`, et y était INATTEIGNABLE depuis
// ailleurs : Next.js refuse au build tout export de valeur depuis un fichier
// `route.ts` (« is not a valid Route export field »), mesuré le 2026-09-12. Une
// génération déclenchée par autre chose que le clic du praticien — la fermeture
// d'un rideau de questionnaires — ne pouvait donc pas réutiliser cette chaîne,
// et aurait dû la recopier. Deux chaînes de génération dans le même dépôt
// finissent par diverger sur ce qu'elles transmettent au modèle : c'est
// exactement ce que ce déménagement empêche.
//
// LE DÉMÉNAGEMENT EST FIDÈLE AU CARACTÈRE PRÈS. Aucune règle n'est touchée,
// aucun texte de consigne réécrit, aucun ordre changé. Les bancs de garde qui
// lisaient le TEXTE SOURCE de la route lisent désormais ce fichier — ils
// assertent les mêmes phrases.
//
// CE QUI RESTE DANS LA ROUTE : la session, l'appartenance, les codes HTTP et
// les deux transports (JSON et SSE). Ce module ne connaît ni l'un ni l'autre —
// il rend un refus typé, l'appelant le traduit.
...
export const MAX_TOKENS_SYNTHESE = 16_384;
...
export async function genererSynthesePersistee(
  args: GenererArgs,
...
export function logErreurGeneration(err: unknown, requestContext: ReturnType<typeof createRequestContext>): void {
  logger.error({
...
export async function auditErreurGeneration(err: unknown, idPatient: string, idSynthese: string): Promise<void> {
...
/**
 * TOUTE LA MATIÈRE D'UNE GÉNÉRATION, LUE ET ASSEMBLÉE — sans un mot de HTTP.
 *
 * Extrait du corps du `POST` sans y toucher : mêmes lectures, mêmes replis
 * best-effort, même ordre. Le seul changement de forme est le refus — la route
 * rendait un 422, ce module rend `{ ok: false, raison }` et laisse traduire.
 *
 * L'APPARTENANCE N'EST PAS VÉRIFIÉE ICI, et c'est délibéré : elle appartient à
 * l'appelant, qui la porte AVANT (la route par `filtrePatientsDuPraticien`).
 * La refaire ici donnerait deux gardes pour une même question, et deux gardes
...
export async function preparerGeneration(
  ...
  emailPatient: string,
  requestContext: RequestContext,
): Promise<{ ok: true; args: GenererArgs } | { ok: false; raison: 'aucune_passation' }> {
    ...
    if (reponsesAdministrables.length === 0) {
      // LE REFUS REMONTE, IL NE SE FORMULE PAS ICI : ce module ne connaît ni
      // HTTP ni code de statut. L'appelant traduit — la route en 422.
      return { ok: false as const, raison: 'aucune_passation' as const };
    }
    ...
    return { ok: true as const, args: genererArgs };
}
