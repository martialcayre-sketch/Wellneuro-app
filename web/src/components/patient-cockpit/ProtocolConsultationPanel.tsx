    ...
    return (
      <section aria-labelledby="patient-protocol-preview-title">
        ...
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-sm font-semibold text-foreground">Aperçu du protocole indisponible — protocole relu et validation pour diffusion requis</p>
          <p className="mt-1 text-sm text-muted-foreground">Aucun contenu n’est transmis au patient.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <div className="grid gap-6">
      <section aria-labelledby="consultation-close-title" className="rounded-xl border border-border bg-surface p-4">
        ...
        <p className="mt-4 text-sm"><span className="font-medium">Raison d’être :</span> {protocolDraft.purpose}</p>
        <p className="mt-2 text-sm"><span className="font-medium">Critère J21 :</span> {protocolDraft.followUpCriterion}</p>
        <p className="mt-2 text-sm"><span className="font-medium">Charge déclarée :</span> {LOAD_LABELS[protocolDraft.therapeuticLoad.level]}</p>
        <ol className="mt-4 grid gap-3">
          {protocolDraft.actions.map((action, index) => (
            <li key={action.actionId} className="rounded-lg border border-border p-3 text-sm">
      ...
      <section aria-labelledby="patient-protocol-preview-title" className="rounded-xl border border-border bg-surface p-4">
        ...
        {!approved ? (
          ...
          <>
            ...
            {previewOpen && (
              <div id="patient-protocol-preview-content" className="mt-4 rounded-lg bg-muted p-4 text-sm">
