...
import { AssignationsPacksPanel } from './AssignationsPacksPanel';
import { PATIENT, stubFetch } from '@/components/dossier/fixturesDossier';

// LE RAYON ASSIGNATIONS ET PACKS — bancs déplacés depuis
// `PatientsPanel.test.tsx` le 2026-09-16, avec leur panneau, et non réécrits :
// une scission qui corrigerait « au passage » ne serait pas relisible.
//
// Trois sujets : l'annulation d'une assignation (Fil A), le compte de journées
// d'agenda porté par la modale, et le filtre de statut avec sa troncature —
// celui dont le défaut d'origine était de filtrer APRÈS le plafond de 40.
...
describe('AssignationsPacksPanel — le tiroir d’assignation (SP-TRAJ LOT-05)', () => {
  ...
  it('le tiroir assignation n’offre plus de pack suggéré, et reste un tiroir d’assignation', async () => {
    stubFetch();
    render(<AssignationsPacksPanel />);
    // L'attente porte sur le DÉCLENCHEUR DU TIROIR, et plus sur « Gérer le
    // dossier » : ce panneau n'affiche plus le tableau des patients depuis la
    // scission du 2026-09-16, donc ce bouton-là n'y existe pas. Attendre un
    // élément absent aurait fait échouer le banc pour une raison sans rapport
    // avec ce qu'il garde.
    await screen.findByRole('button', { name: 'Nouvelle assignation' });
...
describe('AssignationsPacksPanel — annulation d’une assignation (Fil A)', () => {
