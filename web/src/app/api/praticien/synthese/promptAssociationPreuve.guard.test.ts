...
describe('consigne système — une association ne se restitue pas en preuve (DC-09)', () => {
  ...
  it('la formule épinglée est présente, mot pour mot', () => {
  ...
  it('la clause dit ce qui reste ATTENDU, pas seulement ce qui est interdit', () => {
    ...
    expect(CONSIGNE).toMatch(/expliciter un motif ou une hypothèse qui t'est donné reste attendu/i);
  });
  ...
  it('la clause vit dans le CADRE DÉONTOLOGIQUE, au-dessus des sections topiques', () => {
    ...
    const positionClause = CONSIGNE.indexOf('Une association n\'est pas une preuve');
    ...
    expect(positionCadre).toBeGreaterThanOrEqual(0);
    expect(positionPremiereSection).toBeGreaterThan(positionCadre);
    expect(positionClause).toBeGreaterThan(positionCadre);
    expect(positionClause).toBeLessThan(positionPremiereSection);
  });
});
