...
export async function POST(req: Request) {
  ...
  try {
    ...
    // corrige côté client, et elle doit le dire. Aucun bilan partiel ne
    // l'accompagne : une écriture interrompue ne rend pas d'inventaire, et le
    // geste de reprise n'en demande pas — rejouer le lot entier suffit, car
    // `ingestCompositions` est idempotent (un produit déjà écrit revient en
    // `produitsInchanges`). Une version antérieure promettait cet inventaire
